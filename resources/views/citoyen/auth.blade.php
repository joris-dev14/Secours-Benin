...
<head>
    ...
    <style>
        ...
        .phone-field { border: none; min-height: 52px; }
...
<body>
    ...
    async function sendOTP() {
        ...
        if (!/^\d{8}$/.test(phone) && !/^01\d{8}$/.test(phone)) {
            document.getElementById('error-phone').textContent = "Veuillez entrer un numéro valide : 51381904 ou 0151381904.";
        ...
        if (response.ok && data.success) {
            ...
            if (/^\d{8}$/.test(phone)) {
                displayPhone = '01' + phone;
            }
            document.getElementById('display-phone').textContent = "+229 " + displayPhone;
            document.getElementById('step-phone').classList.add('hidden');
            document.getElementById('step-otp').classList.remove('hidden');
            document.querySelector('.otp-input').focus();

            // Affiche le code OTP pour les tests / sandbox
            if (data.sandbox) {
                alert('Mode sandbox : code OTP de test = ' + data.code + '\nLe SMS n\'est pas envoyé sur un vrai numéro en sandbox.');
            } else {
            ...
            document.getElementById('error-phone').textContent = data.message || "Erreur lors de l'envoi.";
            document.getElementById('error-phone').classList.remove('d-none');
        }
    }
